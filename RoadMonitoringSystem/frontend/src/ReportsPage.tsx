...
export default function ReportsPage(props: { userId: number; selectedSegmentId: number | null }) {
  ...
  async function refresh() {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/segments/${props.selectedSegmentId}/reports`
      );
      const text = await res.text();
      if (!res.ok) throw new Error(`GET reports failed: HTTP ${res.status}\n${text}`);
      setReports(JSON.parse(text));
    } catch (e: any) {
      setErr(e?.message ?? "Failed to load reports");
    }
  }

  // ✅ 关键修复：POST reports 用 query params（匹配后端）
  async function create() {
    ...
    try {
      const url = `http://localhost:8000/segments/${props.selectedSegmentId}/reports?${qs.toString()}`;
      const res = await fetch(url, { method: "POST" });
      const text = await res.text();
      if (!res.ok) throw new Error(`POST report failed: HTTP ${res.status}\n${text}`);
  ...
  async function confirm(reportId: number) {
    ...
    try {
      const res = await fetch(`http://localhost:8000/reports/${reportId}/confirm`, {
        method: "PATCH",
      });
      const text = await res.text();
      if (!res.ok) throw new Error(`PATCH confirm failed: HTTP ${res.status}\n${text}`);
          ...
          >
            <div>
              ...
              <div style={{ color: "#555", marginTop: 6, fontSize: 12 }}>
                confirmed: <b>{String(r.confirmed)}</b> | author: {r.author_id}
              </div>
            </div>
